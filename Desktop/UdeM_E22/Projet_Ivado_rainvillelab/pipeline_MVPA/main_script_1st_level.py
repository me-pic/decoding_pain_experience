...
dir_to_save = r'/data/rainville/dylan_projet_ivado_decodage/results_glm_all_shocks'
...
for subj_path in ls_subj_path:
    ...
    for file in [i for i in os.listdir(subj_path) if i.startswith('02') or i.startswith('03')]:
        ...
        timestamps_path, DM_name, df_mvmnt_reg, condition = adapt_data(subj_path, file, subj_name, subj_data, timestamps_path_root)
        ...
        if os.path.exists(os.path.join(dir_to_save,subj_name,done_file_name)) == False : #to enter the contrast section:
            ...
            from make_contrast_all_shocks import glm_contrast_all_shocks

            glm_contrast_all_shocks(design_matrix, os.path.join(dir_to_save,subj_name), subj_name, fmri_img, run = condition)
