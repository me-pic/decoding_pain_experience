...
#-----------------------------
def glm_contrast_1event(design_matrices, subj_result_path, subj_name,fmri_img, run = None):
    ...
    #Arguments :
    #design_matrices : a list of design matrices, one corresponding to each run
    ...
    #vector is a 'number of regressor'(design_matrices.shape[1]) long.
    ...
    for keys in none_contrasts:
        ...
        if type(keys) is str: #if the key is str, in order to exclude the drifts and other parameters
    ...
    return beta_map, contrast_paths

#-----------------------------
def glm_contrast_all_shocks(design_matrices,fmri_img, subj_result_path, subj_name, run = None):
...
def glm_contrast_runs_N_shocks(design_matrices,all_runs_fmri_img, subj_result_path, subj_name, run_name = None):
    ...
    identity_matrix = np.eye((design_matrices[0].shape)[1],(design_matrices[0].shape)[1]) #array size regressors x regressors of the DM
    null_matrix = np.zeros(design_matrices[0].shape)

    none_contrasts = dict([(column, null_matrix[i])
      for i, column in enumerate(design_matrices[0].columns)]) # a dict where all the keys are the DM regressors and their value is a vector [0 0 0 ... 0 0 0]
    #print('NONE CONTRAST  {}'.format(none_contrasts))
    #print('lenght of NONE CONTRAST  {}'.format(len(none_contrasts)))
    ...
    ls_keys = [] #list of all the regessors that will be included in the statistical map
    key_list = list(none_contrasts) #list of all the regessors
    indx = 0
    for keys in none_contrasts:
    ...
    print(ls_keys, ' : All the regressors included in the contrast map')
    #--------computing contrast---------
    print('==============')
    print('COMPUTING CONTRAST')
    print('With the CONTRAST VECTOR : {} '.format(contrast_vector))
...
#-----------------------------
def glm_contrast_runs_all_shocks(design_matrices_paths,all_runs_fmri_img, dir_to_save, subj_name, run_name = None):
    ...
    #print('fmri.shape : {} '.format(all_runs_fmri_img.shape))
    from nilearn.image import mean_img
    ...
    design_matrices = [pd.DataFrame(pd.read_csv(df)) for df in design_matrices_paths]
    ...
    if run_name == None:#control if a run_name string has been provided to include in the name of the file to save
        name_to_save = 'beta_map_' + subj_name  + '_all_shocks'
        contrast_path = os.path.join(dir_to_save, name_to_save)
        nib.save(beta_map, contrast_path)

    if run_name != None:
        name_to_save = 'beta_map_' + subj_name + '_' + run_name + '_all_shocks'
        contrast_path = os.path.join(dir_to_save,subj_name, name_to_save)
        nib.save(beta_map, contrast_path)
    print('Have saved beta_map as a : {} , having shape : {} , under name : {}'.format(type(beta_map),beta_map.shape, name_to_save))
